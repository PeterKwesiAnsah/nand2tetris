...
void translateA(struct Tokens *ins){
    int i;
    int count=ins->count;
    for(i=0;i<count;i++){
...
void translateC(struct Tokens *ins){
    int i;
    int count=ins->count;
    for(i=0;i<count;i++){
        if(!(ins->darray[i]->tokenKind==C))
            continue;
        char hackMachineCode[HACK_MACHINE_WORD+1];
        memset(hackMachineCode, '0', HACK_MACHINE_WORD );
        hackMachineCode[0]='1';
        ...
        int compIndex=0,destIndex=0,jumpIndex=0;
        //Find instruction
        while (compIndex < COMP_TABLE_SIZE && strcmp(comp,comp_table[compIndex++].key));
        while (destIndex < DEST_TABLE_SIZE && strcmp(dest,dest_table[destIndex++].key));
        while (jumpIndex < JUMP_TABLE_SIZE && strcmp(jump,jump_table[jumpIndex++].key));
        //conditional moves
        if(compIndex < COMP_TABLE_SIZE){
            memcpy(hackMachineCode+COMP_HACK_START_INDEX,comp_table[compIndex].value , COMP_HACK_LEN);
        }
        if(destIndex < DEST_TABLE_SIZE){
            memcpy(hackMachineCode+DEST_HACK_START_INDEX,dest_table[destIndex].value, DEST_HACK_LEN);
        }
        if(jumpIndex < JUMP_TABLE_SIZE){
